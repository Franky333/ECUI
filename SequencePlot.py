...
class SequencePlot(FigureCanvas):
	...
	def __init__(self, parent=None, width=5, height=4, dpi=100, sequence=None, countdownTimer=None):
		...
		self.axes.set_title("Fuel and Oxidizer Valve Sequence")
		self.axes.set_xlabel("Time in Seconds")
		self.axes.set_ylabel("Valve Opening in Percent")
		...
		# self.axes.grid()  #FIXME: causes flickering
		...
		# self.axes.legend(handles=[self.fuelSequence, self.oxidizerSequence], loc='upper left')  #FIXME: causes flickering
		self.draw()
